<div align="center">

# ░▒▓█ MATT FROM NICE DREAMZ LLC █▓▒░

<pre>
┌────────────────────────────────────────────────────────────────┐
│ ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░│
│ ░░  Humboldt County, California  |  Nice Dreamz LLC      ░░   │
│ ░░  Building software and learning as I go                ░░   │
│ ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░│
└────────────────────────────────────────────────────────────────┘
</pre>

[![GitHub followers](https://img.shields.io/github/followers/nicedreamzapp?style=for-the-badge)](https://github.com/nicedreamzapp)
[![Total Stars](https://img.shields.io/badge/Total%20Stars-24+-yellow?style=for-the-badge)]()
[![Repos](https://img.shields.io/badge/Public%20Repos-17-blue?style=for-the-badge)]()

## ▓▒░ FEATURED PROJECTS ░▒▓

</div>

---

## 🦞 AI AUTOMATION

<table>
<tr>
<td width="50%" align="center">

### 🦞 ClawdBot

**24/7 AI Business Automation**

Moltbot-powered assistant for e-commerce • WooCommerce integration • Auto order monitoring • Social media drafts • Runs on Mac mini M4

⭐ **NEW** | [View on GitHub →](https://github.com/nicedreamzapp/ClawdBot)

`moltbot` `claude-ai` `automation` `woocommerce`

</td>
<td width="50%" align="center">

### 🤖 divine-tribe-chatbot

**AI Chatbot for ineedhemp.com**

Mistral 7B • RLHF fine-tuned • Hybrid RAG/CAG • Claude-powered • 134 products • 24/7 support

⭐ **8 Stars** | [View on GitHub →](https://github.com/nicedreamzapp/divine-tribe-chatbot)

`mistral-7b` `rlhf` `chatbot` `e-commerce`

</td>
</tr>
</table>

---

## 📱 MOBILE APPS

<table>
<tr>
<td width="50%" align="center">

### 📱 RealTimeAICam

**Largest iPhone Object Detection - FREE**

601 objects • Works 100% offline • OCR text reading • LiDAR depth • No subscription

⭐ [GitHub](https://github.com/nicedreamzapp/RealTimeAICam) | [App Store](https://apps.apple.com/us/app/realtime-ai-cam/id6751230739)

<img src="https://raw.githubusercontent.com/nicedreamzapp/RealTimeAICam/main/EA08B469-F8A2-435D-A3B3-44AEF833E38E.png" width="350">

`yolov8` `ios` `swift` `coreml` `lidar`

</td>
<td width="50%" align="center">

### 👁️ VisionBuilder

**Vision Development Tools for iOS**

Swift framework for building computer vision apps • Metal optimized • Apple Silicon

[View on GitHub →](https://github.com/nicedreamzapp/VisionBuilder)

`swift` `vision` `ios` `metal`

</td>
</tr>
</table>

---

## 🏠 FAMILY & HOME

<table>
<tr>
<td width="50%" align="center">

### 📅 Family Planner Dashboard

**Skylight-inspired Family Command Center**

Voice control • OCR scanning • AI assistant • Seasonal themes • Runs on old tablets

[View on GitHub →](https://github.com/nicedreamzapp/Family-Planner)

<img src="https://raw.githubusercontent.com/nicedreamzapp/Family-Planner/main/screenshot.png" width="350">

`family` `dashboard` `voice-control` `ocr`

</td>
<td width="50%" align="center">

### 🔥 Heat-N-Clean Glass Oven

**Automated Glass Cleaning Kiln**

Custom built glass cleaning solution • Temperature control • Automation

⭐ **NEW** | [View on GitHub →](https://github.com/nicedreamzapp/Heat-N-Clean-Glass-Oven)

`hardware` `automation` `arduino`

</td>
</tr>
</table>

---

## 🤖 ROBOTICS

<table>
<tr>
<td width="50%" align="center">

### 🐔 CemaniHomesteadRobot

**Robot That Guards Chickens**

Tank drive chassis • Xbox controller • YOLOv8 object detection • Predator deterrent • Built from scratch

[View on GitHub →](https://github.com/nicedreamzapp/CemaniHomesteadRobot)

https://github.com/user-attachments/assets/6a05e239-ce66-46ee-b951-474730370bfe

`robotics` `yolov8` `arduino` `opencv`

</td>
<td width="50%" align="center">

### 🗣️ SpeakAnywhere

**Voice Control for Your Computer**

Tired of typing? Just speak • Whisper integration • Works anywhere

[View on GitHub →](https://github.com/nicedreamzapp/SpeakAnywhere)

`whisper` `voice` `accessibility` `python`

</td>
</tr>
</table>

---

## 🧬 RESEARCH & SCIENCE

<table>
<tr>
<td width="50%" align="center">

### 🧬 Parkinson's Vulnerability Predictor

**ML for Parkinson's Disease Research**

100% accuracy on test set • 65,000+ cells analyzed • 20-gene signature • Published methodology

⭐ **1 Star** | [View on GitHub →](https://github.com/nicedreamzapp/parkinsons-vulnerability-predictor)

<img src="https://raw.githubusercontent.com/nicedreamzapp/parkinsons-vulnerability-predictor/main/figures/validation/cross_dataset_vulnerability_comparison.png" width="350">

`machine-learning` `medical` `research` `python`

</td>
<td width="50%" align="center">

### 🎬 CogVideoX-Mac-Setup

**AI Video Generation on Mac**

CogVideoX-5B • Apple Silicon M4 Pro • 4-second videos in 18 minutes • Full setup guide

[View on GitHub →](https://github.com/nicedreamzapp/CogVideoX-Mac-Setup)

https://github.com/user-attachments/assets/f756588c-2bfa-4a37-af1d-1577b85fd01a

`ai-video` `cogvideox` `apple-silicon` `diffusion`

</td>
</tr>
</table>

---

## 💼 BUSINESS TOOLS

<table>
<tr>
<td width="50%" align="center">

### 📱 Social Media Marketing Hub

**Automates Social Media Posts**

20 min → 3-5 min tasks • Reddit, Facebook, Instagram, Twitter, Craigslist • Saves hours weekly

[View on GitHub →](https://github.com/nicedreamzapp/Dreamz-Social-Media-Marketing-Hub)

<img src="https://raw.githubusercontent.com/nicedreamzapp/Dreamz-Social-Media-Marketing-Hub/refs/heads/main/MArketingHubHomepage.png" width="350">

`marketing` `automation` `social-media` `flask`

</td>
<td width="50%" align="center">

### 💰 BitcoinPredictor

**Real-time Bitcoin Trading with AI**

Live prices & predictions • Data streaming • Trading signals • Web dashboard

[View on GitHub →](https://github.com/nicedreamzapp/BitcoinPredictor)

<img src="https://raw.githubusercontent.com/nicedreamzapp/BitcoinPredictor/refs/heads/main/BitTraderUiScreen.png" width="350">

`bitcoin` `trading` `ai` `websockets`

</td>
</tr>
</table>

---

## 🛠️ DEVELOPER TOOLS

<table>
<tr>
<td width="50%" align="center">

### 🛠️ apiai-hub

**1-Click Xcode Dev Tools**

10 min → 1 min tasks • iPhone automation • Build helpers • Code signing shortcuts

[View on GitHub →](https://github.com/nicedreamzapp/apiai-hub)

<img src="https://raw.githubusercontent.com/nicedreamzapp/apiai-hub/main/assets/preview.webp" width="350">

`xcode` `ios` `developer-tools` `automation`

</td>
<td width="50%" align="center">

### 🎨 MattPaint

**MS Paint for the Web**

Pixel-perfect Windows 10 Paint clone • Zero dependencies • Pure vanilla JS • Works in browser

[View on GitHub →](https://github.com/nicedreamzapp/MattPaint)

<img src="https://raw.githubusercontent.com/nicedreamzapp/MattPaint/main/screenshot.png" width="350">

`javascript` `canvas` `paint` `web-app`

</td>
</tr>
</table>

---

## 🌐 WEBSITES & FAN PROJECTS

<table>
<tr>
<td width="50%" align="center">

### 👁️ Disclosure Day Fan Hub

**AI-powered Movie Fan Site**

48 SEO pages • THE SIGNAL chatbot • Live countdown • Full movie database

🔴 **LIVE** | [Visit Site](https://disclosureday.nicedreamzwholesale.com) | [GitHub](https://github.com/nicedreamzapp/DisclosureDay)

[![Disclosure Day](https://img.youtube.com/vi/UFe6NRgoXCM/maxresdefault.jpg)](https://disclosureday.nicedreamzwholesale.com)

`seo` `chatbot` `fan-site` `flask`

</td>
<td width="50%" align="center">

### 🎨 Stickers You Love

**Custom Sticker Business Site**

Built for a friend • E-commerce ready • Clean design

[View on GitHub →](https://github.com/nicedreamzapp/Stickers-You-Love)

`e-commerce` `web-design` `small-business`

</td>
</tr>
</table>

---

<div align="center">

## ▓▒░ ALL 17 REPOSITORIES ░▒▓

| Project | Description | Stars |
|---------|-------------|-------|
| 🦞 [ClawdBot](https://github.com/nicedreamzapp/ClawdBot) | 24/7 AI business automation for Mac mini M4 | ⭐ NEW |
| 🔥 [Heat-N-Clean-Glass-Oven](https://github.com/nicedreamzapp/Heat-N-Clean-Glass-Oven) | Automated glass cleaning kiln | ⭐ NEW |
| 📅 [Family-Planner](https://github.com/nicedreamzapp/Family-Planner) | Skylight-inspired family dashboard | - |
| 🎨 [Stickers-You-Love](https://github.com/nicedreamzapp/Stickers-You-Love) | Custom sticker business site | - |
| 👁️ [DisclosureDay](https://github.com/nicedreamzapp/DisclosureDay) | Disclosure Day movie fan hub | - |
| 🐔 [CemaniHomesteadRobot](https://github.com/nicedreamzapp/CemaniHomesteadRobot) | Chicken-guarding robot | - |
| 🤖 [divine-tribe-chatbot](https://github.com/nicedreamzapp/divine-tribe-chatbot) | AI chatbot for ineedhemp.com | ⭐ 8 |
| 🧬 [parkinsons-vulnerability-predictor](https://github.com/nicedreamzapp/parkinsons-vulnerability-predictor) | ML for Parkinson's research | ⭐ 1 |
| 🎨 [MattPaint](https://github.com/nicedreamzapp/MattPaint) | MS Paint clone for web | - |
| 🗣️ [SpeakAnywhere](https://github.com/nicedreamzapp/SpeakAnywhere) | Voice control for computers | - |
| 👁️ [VisionBuilder](https://github.com/nicedreamzapp/VisionBuilder) | iOS vision development tools | - |
| 📱 [RealTimeAICam](https://github.com/nicedreamzapp/RealTimeAICam) | 601-object detection iPhone app | - |
| 🎬 [CogVideoX-Mac-Setup](https://github.com/nicedreamzapp/CogVideoX-Mac-Setup) | AI video generation on Mac | - |
| 📱 [Dreamz-Social-Media-Marketing-Hub](https://github.com/nicedreamzapp/Dreamz-Social-Media-Marketing-Hub) | Social media automation | - |
| 🛠️ [apiai-hub](https://github.com/nicedreamzapp/apiai-hub) | 1-click Xcode tools | - |
| 💰 [BitcoinPredictor](https://github.com/nicedreamzapp/BitcoinPredictor) | Real-time Bitcoin trading AI | - |
| 📄 [nicedreamzapp](https://github.com/nicedreamzapp/nicedreamzapp) | This profile README | ⭐ 15 |

---

## ▓▒░ TECH STACK ░▒▓

| Category | Technologies |
|----------|-------------|
| **Languages** | Python • Swift • TypeScript • JavaScript • C • C++ • PHP |
| **AI/ML** | Claude • Mistral 7B • YOLOv8 • CogVideoX • RLHF • OpenAI • Whisper • Moltbot |
| **Mobile** | iOS • Xcode • Metal • Apple Silicon • LiDAR • OCR • CoreML |
| **Web** | Flask • HTML5 • CSS3 • JavaScript • PHP • Nginx |
| **Robotics** | Arduino • Motors • Modbus • Object Detection • Tank Drive |
| **Hardware** | Mac mini M4 • Jetson Nano • Raspberry Pi • Custom Kilns |

---

## ▓▒░ LIVE SITES ░▒▓

| Site | Description |
|------|-------------|
| 🌐 [nicedreamzwholesale.com](https://nicedreamzwholesale.com) | Main business site |
| 🌿 [ineedhemp.com](https://ineedhemp.com) | Divine Tribe vaporizers + AI chatbot |
| 🎬 [disclosureday.nicedreamzwholesale.com](https://disclosureday.nicedreamzwholesale.com) | Disclosure Day movie fan hub |
| 🛠️ [tools.marijuanaunion.com](https://tools.marijuanaunion.com) | Marketing Hub tools |

---

## ▓▒░ THE STORY ░▒▓

**Started with chickens, ended up building AI**

```
One problem (protecting chickens)
    → learned robotics
        → built iPhone apps
            → created medical AI
                → automated business
                    → kept building
```

*"If it takes more than 10 minutes, I try to automate it. If it needs AI, I try to build it from scratch."*

---

## ▓▒░ CONNECT ░▒▓

🌐 [nicedreamzwholesale.com](https://nicedreamzwholesale.com) • 📧 [info@nicedreamzwholesale.com](mailto:info@nicedreamzwholesale.com)

💻 [GitHub](https://github.com/nicedreamzapp) • 📸 [Instagram @nicedreamzwholesale](https://instagram.com/nicedreamzwholesale)

---

[![GitHub followers](https://img.shields.io/github/followers/nicedreamzapp?style=social)](https://github.com/nicedreamzapp)
[![GitHub stars](https://img.shields.io/github/stars/nicedreamzapp?style=social)](https://github.com/nicedreamzapp)

---

░▒▓█ NICE DREAMZ LLC █▓▒░

*Building things that hopefully help people*

**Humboldt County, California** 🌲

</div>
